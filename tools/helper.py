...
def goat(args, data, target, feature_1, feature_2, gcn, attn_encoder, device):
    if args.use_goat:
        ...
        video_1_fea = torch.cat([feature_1[:, i:i + 60].mean(1).unsqueeze(1) for i in range(0, 540, 60)], 1)  # B,9,1024
        video_2_fea = torch.cat([feature_2[:, i:i + 60].mean(1).unsqueeze(1) for i in range(0, 540, 60)], 1)  # B,9,1024
...
def network_forward_train(base_model, psnet_model, decoder, regressor_delta, pred_scores,
    ...
    com_feature_12_u = torch.cat((video_1_fea, video_2_fea), 0)  # (2B, 9, 1024)
    com_feamap_12_u = torch.cat((video_1_feamap_re, video_2_feamap_re), 0)  # (32B, 9, 1024)
